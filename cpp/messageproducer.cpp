...
PREPARE_LOGGING(messageproducer_i)

messageproducer_i::messageproducer_i(const char *uuid, const char *label) :
    messageproducer_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

messageproducer_i::~messageproducer_i()
{
}

void messageproducer_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void messageproducer_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &messageproducer_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &messageproducer_i::<callback method>)
        in the constructor.
        ...
        //Add to messageproducer.cpp
        messageproducer_i::messageproducer_i(const char *uuid, const char *label) :
            messageproducer_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &messageproducer_i::scaleChanged);
            addPropertyListener(status, this, &messageproducer_i::statusChanged);
        }

        void messageproducer_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void messageproducer_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to messageproducer.h
...
************************************************************************************************/
int messageproducer_i::serviceFunction()
{
